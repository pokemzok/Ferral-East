...
# FIXME apply reload logic here as well
# as well as gun enchanting parts (like bullet speed or mag or no reload)
# later on I might want to create common Weapon interface, so the weapons could be swapped easily
func attack(attacker: Node2D, target_position: Vector2):
	var bullet = fire_with(attacker, target_position)
	attacker.get_tree().get_root().call_deferred("add_child", bullet)

func fire_with(character: Node2D, target_position: Vector2):
