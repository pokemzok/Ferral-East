...
# 4. better enemy pathfinding + arcade level design -> this would corelate with a better map design
#	possible idea:  leave default zombies without changes but introduce new enemy with new  path-finding behaviour.
# 5. Arcade logic on test map: dynamically add enemies in some sort of a wave logic. 
# 6. I might show the wave number on screen and how many enemies are left to defeat
